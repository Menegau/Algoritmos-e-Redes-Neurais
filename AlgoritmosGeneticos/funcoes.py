...
def Himmelblau(x,y):
    """Função de Himmelblau, simplesmente.
    ...
    Return:
        Cálculo da função para tais X e Y.
    """
...
def funcao_objetivo_himmelblau(individuo):
    """Computa a função objetivo no problema de Himmelblau:
    
    Args:
        Himmelblau: função a ser trabalhada
        individuo: lista contendo os genes das incógnitas a serem testadas
...
def gene_him():
    """ Cria um float aleatório entre 0 e 10.
...
def individuo_him(n_genes):
    """ Cria um candidato para o problema de Himmelblau com dois genes.
    
    Args:
        n_genes: valor float aleatório de 0 a 10
    
    Return:
        Candidato de coordenadas (x,y)
    """
...
def populacao_him(Tamanho,Numero):
    """ Cria uma população para o problema de Himmelblau.
    
    Args:
        Tamanho: quantidade de indivíduos
        Número de Genes: Quantidade de genes em cada indivíduo.
    
    Returns:
        População contendo a quantidade de indivíduos e o número de genes em cada um.
    """
    
    populacao = []
    
    for _ in range(Tamanho):
        populacao.append(individuo_him(Numero))
    
    return populacao

def selecao_torneio_min_him(populacao, fitness, tamanho_torneio=3):
